...
import { User, Calendar, Loader2, Briefcase, ChevronLeft, Star, Award } from 'lucide-react';
          ...
          <CardContent className="p-6">
            <div className="flex flex-col md:flex-row gap-8 items-start">
              ...
              <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
                {ratingCount > 0 && (
                  <div className="col-span-1 md:col-span-2 bg-amber-50 border border-amber-100 rounded-xl p-4 flex items-center gap-4 mb-2">
                    <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center text-amber-500 shadow-sm shrink-0">
                      <Star className="fill-amber-400" size={24} />
                    </div>
                    <div>
                      <h3 className="font-bold text-gray-900 text-lg flex items-center gap-2">
                        {rating.toFixed(1)} / 5.0
                        {rating >= 4.5 && <span className="bg-amber-100 text-amber-800 text-xs px-2 py-0.5 rounded-full flex items-center gap-1"><Award size={12}/> Trabajador Excelente</span>}
                      </h3>
                      <p className="text-gray-600 text-sm">Basado en {ratingCount} turnos completados</p>
                    </div>
                  </div>
                )}
